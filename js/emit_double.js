...
emit_double(u, p, small_font)
{
	...
	if (i == -1)
		emit_roman(u, s.substring(0, k), small_font);
	else {
		for (j = k - 1; j > i + 1; j--) {
			if (s[j] != '0')
				break;
		}
		emit_roman(u, s.substring(0, j + 1), small_font);
	}
	...
	emit_roman(v, s.substring(k, s.length - 1), 1);
