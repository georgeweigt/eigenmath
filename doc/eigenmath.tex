...
Returns $f$ evaluated at $x$ equals $a$, $y$ equals $b$, etc.
All of the arguments can be expressions.
...
For example, \verb$eval(a b c, b c, 1)$ does not match, returns
\verb$a b c$.
...
\item
The following exercise demonstrates some \verb$eval$ tricks.
Let\footnote{From exercise 4-10 of {\it Quantum Mechanics} by Richard Fitzpatrick.}
\begin{equation*}
\psi
...
Note: \verb$eval$ uses exact pattern matching, not arithmetic matching.
For example, \verb$eval(a b c, b c, 1)$ does not match and returns
\verb$a b c$.
