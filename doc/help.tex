...
Multiple roots are returned as a vector.
If no roots are found then symbol \verb$nil$ is returned.
See also \verb$nroots$.
...
\begin{verbatim}
p = x^2 + 1
infixform(roots(p,x)) -- use infixform because nil results are not printed
\end{verbatim}
}
...
\noindent
Note: For macOS, {\it file} must be in the Downloads folder due to security requirements for apps distributed on the Mac App Store.
