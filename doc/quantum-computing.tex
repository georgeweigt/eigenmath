...
\end{equation*}
The final step is to measure $\psi$ and get a result.
Measurement rotates $\psi$ to an eigenstate $|j\rangle$.
The measurement result is $|j\rangle$.
The probability of obtaining a specific $|j\rangle$ is
\begin{equation*}
P_j=c_jc_j^*
\end{equation*}
The measurement result can only be an eigenstate.
In general the coefficients $c_j$ cannot be observed.
