...
extern int get_xheight(int);
extern int get_width(int, int);
...
static void cm_cm_emit_number(struct atom *);
...
static struct {
	...
	{"alpha",	ITALIC_SYMBOL_FONT,	},
	{"beta",	ITALIC_SYMBOL_FONT,	},
	{"gamma",	ITALIC_SYMBOL_FONT,	},
	{"delta",	ITALIC_SYMBOL_FONT,	},
	{"epsilon",	ITALIC_SYMBOL_FONT,	},
	{"zeta",	ITALIC_SYMBOL_FONT,	},
	{"eta",		ITALIC_SYMBOL_FONT,	},
	{"theta",	ITALIC_SYMBOL_FONT,	},
	{"iota",	ITALIC_SYMBOL_FONT,	},
	{"kappa",	ITALIC_SYMBOL_FONT,	},
	{"lambda",	ITALIC_SYMBOL_FONT,	},
	{"mu",		ITALIC_SYMBOL_FONT,	},
	{"nu",		ITALIC_SYMBOL_FONT,	},
	{"xi",		ITALIC_SYMBOL_FONT,	},
	{"omicron",	ITALIC_SYMBOL_FONT,	},
	{"pi",		ITALIC_SYMBOL_FONT,	},
	{"rho",		ITALIC_SYMBOL_FONT,	},
	{"sigma",	ITALIC_SYMBOL_FONT,	},
	{"tau",		ITALIC_SYMBOL_FONT,	},
	{"upsilon",	ITALIC_SYMBOL_FONT,	},
	{"phi",		ITALIC_SYMBOL_FONT,	},
	{"chi",		ITALIC_SYMBOL_FONT,	},
	{"psi",		ITALIC_SYMBOL_FONT,	},
	{"omega",	ITALIC_SYMBOL_FONT,	},

	{"hbar",	ITALIC_SYMBOL_FONT,	},
};

#define MULTIPLICATION 177
#define PARTIAL 178
#define MINUS 179
...
cm_emit_multiply(struct atom *p, int n)
{
	...
	if (n == 0) {
		...
		if (isplusone(car(p)) || isminusone(car(p)))
			p = cdr(p);
		while (iscons(p)) {
			state = (10 * state + (isintegerfactor(car(p)) ? 1 : 2)) % 100;
			switch (state) {
			case 11:
				// adjacent numerical factors
				cm_emit_thin_space();
...
cm_emit_fraction(struct atom *p, int d)
{
	...
	while (iscons(p1)) {
		p2 = car(p1);
		if (!cm_isdenominator(p2)) {
			state = (10 * state + (isintegerfactor(p2) ? 1 : 2)) % 100;
			switch (state) {
			case 11:
...
//		cm_emit_char(SYMBOL_FONT, PARTIAL);
	...
	if (n == -1)
		...
		cm_emit_char(greek[n].font, n + 128);
		s += strlen(greek[n].s);
	}
...
cm_emit_str(int font, char *s)
{
	...
	chartab[indx].x = xpos;
	chartab[indx].y = -get_ascent(font);
	chartab[indx].h = h;
	chartab[indx].w = w;
	xpos += w;
	indx++;
...
static void
cm_cm_emit_number(struct atom *p)
{
	int k1, k2;
	char *s;
	static char buf[100];
...
cm_emit_thick_space(void)
{
	int w;
	if (level == 0)
		w = get_width(TIMES_FONT, 'm');
	else
		w = get_width(SMALL_TIMES_FONT, 'm');
	cm_emit_space(w);
}
...
cm_emit_thin_space(void)
{
	int w;
	if (level == 0)
		w = get_width(TIMES_FONT, 'm') / 2;
	else
		w = get_width(SMALL_TIMES_FONT, 'm') / 2;
	cm_emit_space(w);
}
...
cm_emit_very_thin_space(void)
{
	int w;
	if (level == 0)
		w = get_width(TIMES_FONT, 'm') / 6;
	else
		w = get_width(SMALL_TIMES_FONT, 'm') / 6;
	cm_emit_space(w);
}
...
{
	if (indx && chartab[indx - 1].s && chartab[indx - 1].s[0] == (char) MINUS)
		cm_emit_thin_space();
}
...
{
	if (level == 0)
		return get_xheight(TIMES_FONT);
	else
		return get_xheight(SMALL_TIMES_FONT);
...
{
	if (level == 0)
		return get_ascent(TIMES_FONT) / 3 + 1;
	else
		return get_ascent(SMALL_TIMES_FONT) / 3 + 1;
}
...
do_group(int i, int j)
{
	...
	if (y < ymin || y + h > ymax) {
		...
		chartab[i].s = NULL;
		chartab[j].s = NULL;
		chartab[i].cmd = DRAW_LEFT_BRACKET;
		chartab[j].cmd = DRAW_RIGHT_BRACKET;
		chartab[i].y = y;
		chartab[j].y = y;
		chartab[i].h = h;
		chartab[j].h = h;
